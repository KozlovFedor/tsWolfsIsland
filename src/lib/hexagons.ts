...
export class Layout
{
    constructor (public orientation:Orientation, public size:Point, public origin:Point, public gap: number = 0) {}
    ...
    public hexToPixel(h:Hex):Point
    {
        ...
        let gap: number = this.gap;
        let x:number = (M.f0 * h.q + M.f1 * h.r) * (size.x + gap);
        let y:number = (M.f2 * h.q + M.f3 * h.r) * (size.y + gap);
